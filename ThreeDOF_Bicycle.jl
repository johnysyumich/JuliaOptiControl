function ThreeDOFBicycle_expr(states, controls)
  # lateral tire load
  x = states[1]
  y = states[2]
  v = states[3]
  ...
  sr = controls[1]
  jx = controls[2]
  # alphaf = atan((v + la * r) / (ux)) - sa
  # alphar = atan((v - lb * r) / (ux))
  # FZF = FzF0 - (ax - v * r) * KZX
  # FZR = FzR0 + (ax - v * r ) * KZX
  # FYF = (D * FZF * sin(C * atan(alphaf - E * (alphaf - atan(alphaf)))))
  # FYR = (D * FZR * sin(C * atan(alphar - E * (alphar - atan(alphar)))))
