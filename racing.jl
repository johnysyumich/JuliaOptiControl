...
function DefineOCP(cur_states, block_num, nCol)
    ...
    OCPForm = ConfigurePredefined(ocp; (:Np=>nCol), (:tfDV => false), (:tf => 4.0), (:IntegrationScheme=>:bkwEuler), (:dx => ThreeDOFBicycle_expr))
...
for i = 1:1:2000
    ...
    @time begin
        ...
        set_start_value.(ocp.f.mdl[:x], initial_guess[:,1:8])
        set_start_value.(ocp.f.mdl[:u], initial_guess[:,9:10])
        # set_start_value.(ocp.f.mdl[:block_center_x], block_center_x_info)
        # set_start_value.(ocp.f.mdl[:block_center_y], block_center_y_info)
        # set_start_value.(ocp.f.mdl[:block_yaw], block_yaw_info)
        # set_start_value.(ocp.f.mdl[:block_length], block_length_info)
        # set_start_value.(ocp.f.mdl[:block_width], block_width_info)
        # set_start_value.(ocp.f.mdl[:cost_para], poly33_params)
        ...
        fix(x[1], states_real[1]; force = true)
        fix(y[1], states_real[2]; force = true)
        fix(v[1], states_real[3]; force = true)
        fix(r[1], states_real[4]; force = true)
        fix(ψ[1], states_real[5]; force = true)
        fix(ux[1], states_real[6]; force = true)
        fix(sa[1], states_real[7]; force = true)
        fix(ax[1], initial_guess[1, 8]; force = true)
        fix(sr[1], initial_guess[1, 9]; force = true)
        fix(jx[1], initial_guess[1, 10]; force = true)
        # fix(Δt[1], Δt_s; force = true)
