...
"""
# Build frames on a uniform time grid with linear interpolation (dt = 0.1 s)
function create_animation_frames(ocp; dt::Float64=0.1)
    ...
    # Interpolate each state dimension independently
    nstate = size(states, 2)
    new_states = Array{Float64}(undef, length(new_time), nstate)
    for j in 1:nstate
        itp = Interpolations.linear_interpolation(time, states[:, j])  # inside-bounds use only
    ...
    frames = Vector{Plots.Plot{Plots.GRBackend}}()  # or just `frames = []`

    for (frame_num, t) in enumerate(new_time)
        x_cart = new_states[frame_num, 1]
        theta  = new_states[frame_num, 3]
        ...
        p = plot(xlims=(-2.5, 2.5), ylims=(-0.3, 1.2),
                 aspect_ratio=:equal,
                 title="CartPole Animation - Frame $frame_num/$(length(new_time))",
        ...
        annotate!(1.8, 1.0, text("t = $(round(t, digits=2)) s", 12))
...
# Helper to save frames as a GIF at figures/animation.gif
function save_frames_as_gif(frames; fps::Int=30, path::AbstractString="figures/cartpole_animation.gif")
    mkpath(dirname(path))
    anim = Animation()
    for f in frames
        frame(anim, f)
...
if success
    ...
    frames = create_animation_frames(ocp, dt=0.05)
    for f in frames
        display(f)  # Display each frame (optional)
    end
    save_frames_as_gif(frames; fps=20, path="figures/animation.gif")
